...
\definecolor{alizarin}{rgb}{0.82, 0.1, 0.26}%给超链接用
\definecolor{amethyst}{rgb}{0.6, 0.4, 0.8}%给超链接用
\definecolor{lapislazuli}{rgb}{0.15, 0.38, 0.61}%给超链接用
\definecolor{iceberg}{rgb}{0.44, 0.65, 0.82}%给超链接用
\usepackage[colorlinks,linkcolor=black,urlcolor=lapislazuli,anchorcolor=black,citecolor=black,CJKbookmarks=True]{hyperref}%超链接
...
\par
可能你已经发现了，按照上面的代码插入的图片，会经常不显示在自己需要的位置。因为我们设置了浮动方式是htp，\LaTeX 会自动调整图片的位置。这时候我们想强硬一点，就得使用float包，然后把浮动设置为H（注意是大写H）。还有一种方法，我现在正在使用。对于一些不需要使用标题的图片，或者标题放在图片之上也很美观的情况，我们不使用figure的浮动环境，而是使用minipage来插入图片。一个例子，图\ref{pic:ltx} ：
\par
\begin{minipage}{\linewidth}
\makebox[\linewidth]{}
\centering
\captionof{figure}{\LaTeX }
\includegraphics[scale=0.18]{figures/latex.png}
\label{pic:ltx}
\end{minipage}
\par
如果图片过大，会被自动放到下一页，但是不会改变图片和文字的相对位置。这样插图的代码如下：
\par
\begin{lstlisting}[language={[ANSI]C}, numbers=left, numberstyle=\tiny, keywordstyle=\color{blue!70},  frame=shadowbox, rulesepcolor=\color{red!20!green!20!blue!20}]
\begin{minipage}{\linewidth}
\makebox[\linewidth]{}
...
\caption{内容，框线，对齐，单元格合并，轻松搞定~}%`图片的标题`
\label{pic:tb}
\end{figure}
...
\caption{generate生成的代码直接复制，注意按照要求添加package}%`图片的标题`
\label{pic:tb2}
\end{figure}
...
\begin{tabular}{rrrr}
\Xhline {1.0pt}
\multicolumn{1}{!{\vrule width1.2pt}c!{\vrule width0.7pt}}{\multirow {3}{*}{\mgape{\includegraphics[scale=0.3]{Figures/polytech.png}}}}&\multicolumn{2}{c}
...
\makebox[\linewidth]{}
  \centering
  \captionof{table}{Données à tracer le diagramme de Bode}
    \begin{tabular}{lll}
    \toprule
...
\end{lstlisting}
\par
然后我们就可以开始开心地画图了！其实在TexMaker中自带了TikZ包的快捷插入按钮！没错就是在这里：
\par
\begin{minipage}{\linewidth}
\makebox[\linewidth]{}
\centering
\captionof{figure}{按钮位置}
\includegraphics[scale=.66]{figures/ti.png}
\label{pic:ti}
\end{minipage}
\par
...
\makebox[\linewidth]{}
\centering
\captionof{figure}{一个简单时序图，代码也在$CircuitTikZ_Template.tex$里面}
...
\par\href{http://www.codecogs.com/latex/eqneditor.php}{在线写公式}

\href{https://bitbucket.org/rivanvx/beamer/wiki/Home}{Beamer：用\LaTeX 制作PPT}
\par
\href{http://latexcolor.com/}{\LaTeX 颜色预定义集合} （将颜色预定义加到头文件区域的\textcolor[RGB]{145,25,12}{最后}）
\par
\href{http://www.tablesgenerator.com/}{Tables Generator 在线绘制表格}%左边是网站
\chapter{常见问题}
\par
如果你发现编译完毕以后图的序号和链接对应不上，或者目录对应的章节页码有问题，不要担心，请点击按钮再编译一次，应该可以解决（我也不知道为什么会这样= =）!
\par
如果你发现使用加粗、倾斜等命令后没有效果，很有可能是你选择的这句话字体本身不带加粗、倾斜效果。
\par
如果你发现我的头文件区域在编译时出错，十有八九是因为我定义的字体在你系统的字体库里没有安装，注释掉就可以了！
\chapter{更新日志}
27/11/2015 更新：
\par
增加画电路图部分\ref{pge:dlt}节！插图代码！
19/11/2015 更新：
\par
添加了一个绘制表格网站和查询颜色预定义的网站~ 感觉妈妈又更不用担心我的报告了！
\par

11/11/2015 更新：
\par
特别感谢张子谋同志，是他的\LaTeX 贴心教程 给我启蒙。我的大部分技能都是从这份教程里面获得的。

\par
在GitHub同目录下，有一些说明文档（PDF格式），详情请参见GitHub下README文件。
\par
